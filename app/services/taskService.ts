...
export async function addTask(text: string): Promise<Task> {
  await delay(300);
  const stored = localStorage.getItem(STORAGE_KEY);
  const tasks: Omit<Task, "parsedElements">[] = stored ? JSON.parse(stored) : [];
  const newTask: Task = {
  ...
  tasks.push({ id: newTask.id, text: newTask.text, completed: newTask.completed });
  localStorage.setItem(STORAGE_KEY, JSON.stringify(tasks));
  return newTask;
}

export async function updateTask(id: string, newText: string): Promise<Task | null> {
  await delay(300);
  const stored = localStorage.getItem(STORAGE_KEY);
